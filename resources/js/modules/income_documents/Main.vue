    ...
    <div>
        ...
        >
            ...
            <template v-slot:default="slotProps">
                <tr
                    v-for="data in slotProps.paginatedData"
                    :key="data.id"
                ...
                >
                    ...
                    <td class="text-right">
                        {{ data.sum1 | formatNumber(2) }}
                    </td>
                    <td class="text-right">
                        {{ data.sum2 | formatNumber(2) }}
                    </td>
                    <td class="text-right">
                        {{ data.gain | formatNumber(2) }}
                    </td>
                    ...
                    <td class="text-center">
                        <template>
                            <abbr title="Первая форма" v-if="data.firstForm">
            ...
            <template v-slot:footer>
                <tr v-if="selectedRecords.length > 0">
                    <td colspan="6" class="font-weight-italic">
                        Выделено документов: {{ selectedRecords.length }}
                    </td>
                    <td class="text-right font-weight-italic">
                        {{ totalSelectedPurchaseSum | formatNumber(2) }}
                    </td>
                    <td class="text-right font-weight-italic">
                        {{ totalSelectedRetailSum | formatNumber(2) }}
                    </td>
                    <td class="text-right font-weight-italic">
                        {{
                            (totalSelectedRetailSum - totalSelectedPurchaseSum)
                                | formatNumber(2)
                        }}
                    </td>
                    <td colspan="2"></td>
                ...
                <tr>
                    <td colspan="6" class="font-weight-bold">
                        Итого документов: {{ documents.length }}
                    </td>
                    ...
                    <td class="text-right font-weight-bold">
                        {{
                            (totalRetailSum - totalPurchaseSum)
                                | formatNumber(2)
                        }}
                    </td>
                    <td colspan="2"></td>
...
export default {
    ...
    methods: {
        ...
        handleClick(record) {
            this.selectedRecords = this.documents.filter(
    ...
    computed: {
        ...
        totalPurchaseSum() {
            let total = this.documents.reduce((a, b) => a + b.sum1 * 1, 0.0);
            return total;
        },
        totalRetailSum() {
            let total = this.documents.reduce((a, b) => a + b.sum2 * 1, 0.0);
            return total;
        },
        totalSelectedPurchaseSum() {
            let total = this.selectedRecords.reduce(
                (a, b) => a + b.sum1 * 1,
                0.0
            );
            return total;
