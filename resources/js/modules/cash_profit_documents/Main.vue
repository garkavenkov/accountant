...
<div>
     <grid  :dataTable="documents"
        ...
        <template v-slot:title>
            <h3 class="card-title">Доход кассы
        ...
        <template v-slot:default="slotProps">
            <tr v-for="data in slotProps.paginatedData" :key="data.id">
                <td class="text-center">
                    <router-link :to="{name: 'CashProfitDocumentShow', params: {id: data.id}}">
        ...
        <template v-slot:footer>
            <tr>
                ...
                <td class="text-right font-weight-bold">{{totalProfit | formatNumber(2) }}</td>
                <td></td>
            </tr>
        </template>       
    </grid>
